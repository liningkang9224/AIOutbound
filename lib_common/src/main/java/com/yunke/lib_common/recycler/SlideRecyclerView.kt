...
/**
 * @auth : linker
 * @date : 2020/10/21 5:30 PM
 * @description ：带有侧滑功能的RecyclerView
 */
    ...
    override fun onInterceptTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.action) {
            MotionEvent.ACTION_DOWN -> {
                mScroller?.apply {
                    if (isFinished) {  // 如果动画还没停止，则立即终止动画
                        abortAnimation()
                    }
                    run {
                        mLastX = x.toFloat()
                ...
                MotionEvent.ACTION_UP -> {
                    if (mMenuViewWidth != INVALID_CHILD_WIDTH) {
                        ...
                        mVelocityTracker?.apply {
                            ...
                            when {
                                xVelocity < -SNAP_VELOCITY -> {    // 向左侧滑达到侧滑最低速度，则打开
                                    mScroller?.startScroll(scrollX, 0, mMenuViewWidth - scrollX, 0, Math.abs(mMenuViewWidth - scrollX))
                                }
                                xVelocity >= SNAP_VELOCITY -> {  // 向右侧滑达到侧滑最低速度，则关闭
                                    mScroller?.startScroll(scrollX, 0, -scrollX, 0, Math.abs(scrollX))
                                }
                                scrollX >= mMenuViewWidth / 2 -> { // 如果超过删除按钮一半，则打开
                                    mScroller?.startScroll(scrollX, 0, mMenuViewWidth - scrollX, 0, Math.abs(mMenuViewWidth - scrollX))
                                }
                                else -> {    // 其他情况则关闭
                                    mScroller?.startScroll(scrollX, 0, -scrollX, 0, Math.abs(scrollX))
    ...
    override fun computeScroll() {
        mScroller?.let {
            if (it.computeScrollOffset()) {
                mFlingView!!.scrollTo(it.currX, it.currY)
                invalidate()
            }
        }
